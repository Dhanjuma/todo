...
router.post('/create_todo', async (req, res) => {
    const {title, task, owner_id, token, owner_name, deadline, category} = req.body;
    ...
    try{
        ...
        const data = {titles: todo.title, ids: todo._id}
...
router.post('/delete_todo', async (req, res) => {
    ...
    try{
        ...
        return res.status(200).send({status: 'ok', msg: 'delete successful', todo});
    }catch(e){
        console.log(e);
